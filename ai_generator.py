...
def generate_reports(news_text, sheet_data_text, yield_text, fng_text, indices_text):
  ...
  # ... (앞부분 생략) ...
    ...
    [핵심 작성 조건]
    1. 주요 시장 지수의 변화와 VIX, 공포탐욕 지수, 특히 **10년물/30년물 국채 금리 변동**을 종합하여 현재 시장 상황을 분석해.
    2. 개별 종목은 AI 점수 75점 이상만 선별해 추세 및 요점 분석 (75점 미만 철저히 제외)
    3. 뉴스를 활용해 반도체, 빅테크, 거시경제, 암호화폐, 지정학적 리스크 등 주요 이슈를 심층 분석
    ...
    **💡 거시 경제 & 시장 심리 분석:**
    (표 바로 아래에 줄글로 3대 지수 흐름, 공포탐욕 지수 상태, 그리고 **장단기 국채 금리 변동이 증시에 미치는 영향과 의미**를 반드시 상세하게 설명해 줘! 절대 생략하지 마.)
