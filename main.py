...
# =====================================================================
# 4. Gemini API 적용 및 맞춤형 프롬프트
# =====================================================================
client = genai.Client(api_key=os.environ.get("GEMINI_API_KEY"))

prompt = f"""
너는 미국 주식 전문 애널리스트야. 
다음 세 가지 데이터를 종합해서 '{us_date_str} 미국 증시 마감 리뷰 및 종목 분석 리포트'를 마크다운(.md) 포맷으로 작성해.
...
1. 시장 심리 및 거시 지표: [데이터 2]의 '공포탐욕 지수'와 [데이터 3]의 '국채 금리(10년물, 30년물) 변동'을 리포트 최상단에 아주 크고 눈에 띄게 배치해. 장단기 금리 흐름이나 변동폭이 증시와 시장 심리에 미치는 영향을 짧게 코멘트해 줘.
2. 주요 종목 하이라이트 (AI 점수 75점 이상): [데이터 2]를 분석해서 'AI 점수가 75점 이상'인 종목들만 선별해. 그리고 해당 종목들의 '추세 상태(차트 정보)'와 요점을 중심으로 분석 내용을 적어 줘. (75점 미만은 철저히 생략)
3. 거시 경제 및 주요 테마 분석: [데이터 1]의 뉴스를 활용해서 반도체, 빅테크, 암호화폐 규제, 거시경제(전쟁 등 지정학적 리스크) 흐름과 시장 이슈를 심도 있게 분석해 줘. 
...
if bot_token and chat_id:
    ...
    text_to_send = f"🔔 {us_date_str} 맞춤형 분석 리포트!\n\n{response.text[:3800]}..." 
