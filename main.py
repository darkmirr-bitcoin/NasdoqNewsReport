...
1. 시장 심리 (공포탐욕 지수): [데이터 2]에 있는 '공포탐욕 지수' 점수를 리포트 최상단에 아주 크고 눈에 띄게 배치해. 현재 시장의 심리 상태를 짧게 코멘트해 줘.
2. 주요 종목 하이라이트 (AI 점수 75점 이상): [데이터 2]를 분석해서 'AI 점수가 75점 이상'인 종목들만 선별해. 그리고 해당 종목들의 '추세 상태(차트 정보)'와 요점을 중심으로 분석 내용을 적어 줘. (75점 미만은 생략)
3. 거시 경제 및 주요 테마 분석: [데이터 1]의 뉴스를 활용해서 반도체, 빅테크, 금리, 지정학적 리스크(전쟁 등) 같은 주요 거시 경제 흐름과 시장 이슈를 심도 있게 분석해 줘. 
4. 금지 사항: 뉴스나 시트 데이터에 없는 지수 수치나 종목 점수를 임의로 지어내지 마. 철저히 팩트 기반으로만 작성해.
